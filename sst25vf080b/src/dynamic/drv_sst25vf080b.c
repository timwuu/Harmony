/*******************************************************************************
  SST25VF080B SPI Flash Driver Dynamic implemention.
...
#include "driver/spi_flash/sst25vf080b/src/drv_sst25vf080b_local.h"
...
DRV_SST25VF080B_OBJ gDrvSST25VF080BObj[DRV_SST25VF080B_INSTANCES_NUMBER] ;

/* This is the client object array. */
DRV_SST25VF080B_CLIENT_OBJ gDrvSST25VF080BClientObj[DRV_SST25VF080B_CLIENTS_NUMBER];

/* This is the array of SST25VF080B SPI Flash Driver Buffer objects. */
DRV_SST25VF080B_BUFFER_OBJ gDrvSST25VF080BBufferObj[DRV_SST25VF080B_QUEUE_DEPTH_COMBINED];
...
uint16_t gDrvSST25VF080BBufferToken = 0;

#define _DRV_SST25VF080B_ERASE_BLOCK_SIZE               (4096)

uint8_t gDrvSST25VF080BBackupBuffer[_DRV_SST25VF080B_ERASE_BLOCK_SIZE];

/* This object maintains data that is required by all SST25VF080B
   driver instances. */
DRV_SST25VF080B_COMMON_DATA_OBJ gDrvSST25VF080BCommonDataObj;

// *****************************************************************************
// *****************************************************************************
// Section: SST25VF080B SPI Flash Driver Interface Implementations
// *****************************************************************************
// *****************************************************************************

// *****************************************************************************
/* Function:
    DRV_SST25VF080B_CLIENT_OBJ * _DRV_SST25VF080B_DriverHandleValidate(DRV_HANDLE handle)

  Summary:
    Dynamic implementation of the _DRV_SST25VF080B_DriverHandleValidate() function.

  Description:
    Dynamic implementation of the _DRV_SST25VF080B_DriverHandleValidate() function.
    This function returns NULL if the handle is invalid else it return a pointer
    to the SST25VF080B Driver Client Object associated with this handle.
...
DRV_SST25VF080B_CLIENT_OBJ * _DRV_SST25VF080B_DriverHandleValidate(DRV_HANDLE handle)
{
    ...
    DRV_SST25VF080B_CLIENT_OBJ * client;
    ...
    client = (DRV_SST25VF080B_CLIENT_OBJ *)handle;
...
// *****************************************************************************
/* Function:
    SYS_MODULE_OBJ DRV_SST25VF080B_Initialize
    (
  ...
  Summary:
    Dynamic impementation of DRV_SST25VF080B_Initialize system interface function.

  Description:
    This is the dynamic impementation of DRV_SST25VF080B_Initialize system
    interface function.
  
  Remarks:
    See drv_sst25vf080b.h for usage information.
*/
...
{
    DRV_SST25VF080B_OBJ *dObj = (DRV_SST25VF080B_OBJ*)NULL;
    DRV_SST25VF080B_INIT *sst25vf080bInit = NULL ;

    /* Check if the specified driver index is in valid range */
    if(drvIndex >= DRV_SST25VF080B_INSTANCES_NUMBER)
    {
        SYS_DEBUG(0, "DRV_SST25VF080B: Invalid driver index \n");
        return SYS_MODULE_OBJ_INVALID;
    }

    /* Check if this hardware instance was already initialized */
    if(gDrvSST25VF080BObj[drvIndex].inUse != false)
    {
        SYS_DEBUG(0, "DRV_SST25VF080B: Instance already in use \n");
        return SYS_MODULE_OBJ_INVALID;
    }
    
    /* Assign to the local pointer the init data passed */
    sst25vf080bInit = ( DRV_SST25VF080B_INIT * ) init ;

    /* Allocate the driver object and set the operation flag to be in use */
    dObj = &gDrvSST25VF080BObj[drvIndex];
    dObj->inUse = true;
    ...
    dObj->spiDriverModuleIndex = sst25vf080bInit->spiDriverModuleIndex;
    
    dObj->nClients = 0;
    dObj->isExclusive = false;
    
    /* Set the port remapping pins as per init structure */
    dObj->chipSelectPortChannel = sst25vf080bInit->chipSelectPortChannel;
    dObj->chipSelectBitPosition = sst25vf080bInit->chipSelectBitPosition;


    if(_DRV_SST25VF080B_HardwareHoldIsEnabled())
    {
        ...
        SYS_PORTS_PinDirectionSelect(PORTS_ID_0,SYS_PORTS_DIRECTION_OUTPUT, sst25vf080bInit->holdPortChannel, sst25vf080bInit->holdBitPosition);
        /* Keep the HOLD pin always HIGH, it should never have high to low transition */
        SYS_PORTS_PinSet(PORTS_ID_0, sst25vf080bInit->holdPortChannel, sst25vf080bInit->holdBitPosition);
    }

    if(_DRV_SST25VF080B_HardwareWPIsEnabled())
    {
        ...
        SYS_PORTS_PinDirectionSelect(PORTS_ID_0,SYS_PORTS_DIRECTION_OUTPUT, sst25vf080bInit->writeProtectPortChannel, sst25vf080bInit->writeProtectBitPosition);
        /* Keep the WP pin always HIGH so that status refister will be always writeble */
        SYS_PORTS_PinSet(PORTS_ID_0, sst25vf080bInit->writeProtectPortChannel, sst25vf080bInit->writeProtectBitPosition);
    }
    ...
    dObj->queueTail = NULL;
    dObj->queueSize = sst25vf080bInit->queueSize;
    dObj->queueOccupancy = 0;

    /* Set driver task state to opening spi driver */
    dObj->state = DRV_SST25VF080B_TASK_OPEN_SPI_DRIVER;
    
    /* Set internal operations state */
    dObj->bufferProcessState = DRV_SST25VF080B_BUFFER_PROCESS_INIT;
    dObj->eraseWriteState = _DRV_SST25VF080B_ERASE_WRITE_INIT;
...
//       then create these. */
//
//     if(!gDrvSST25VF080BCommonDataObj.membersAreInitialized)
//     {
//         /* This means that mutexes where not created. Create them. */
//         OSAL_ASSERT((OSAL_MUTEX_Create(&(gDrvSST25VF080BCommonDataObj.mutexClientObjects)) == OSAL_RESULT_TRUE),
//                     "Unable to create client instance mutex");
//         OSAL_ASSERT((OSAL_MUTEX_Create(&(gDrvSST25VF080BCommonDataObj.mutexBufferQueueObjects)) == OSAL_RESULT_TRUE),
//                     "Unable to create buffer queue objects mutex");
//
//         /* Set this flag so that global mutexes get allocated only once */
//         gDrvSST25VF080BCommonDataObj.membersAreInitialized = true;
//     }
...
//******************************************************************************
/* Function:
    void DRV_SST25VF080B_Deinitialize( SYS_MODULE_OBJ object )
...
void DRV_SST25VF080B_Deinitialize( SYS_MODULE_OBJ object)
{
    DRV_SST25VF080B_OBJ * dObj;
    DRV_SST25VF080B_BUFFER_OBJ * iterator;


    /* Check that the object is valid */
    if(object == SYS_MODULE_OBJ_INVALID)
    {
        SYS_DEBUG(0, "DRV_SST25VF080B: Invalid system module object \n" );
        return;
    }
    
    if(object >= DRV_SST25VF080B_INSTANCES_NUMBER)
    {
        SYS_DEBUG(0, "DRV_SST25VF080B: Invalid system module object \n" );
        return;
    }

    dObj = (DRV_SST25VF080B_OBJ*) &gDrvSST25VF080BObj[object];
    ...
    /* Deinitialize the SST25VF080B status */
...
//*************************************************************************
/* Function:
    SYS_STATUS DRV_SST25VF080B_Status( SYS_MODULE_OBJ object )
...
SYS_STATUS DRV_SST25VF080B_Status( SYS_MODULE_OBJ object)
{
    ...
    if(object == SYS_MODULE_OBJ_INVALID)
    {
        SYS_DEBUG(0, "DRV_SST25VF080B: Invalid system object handle \n");
    ...
    return (gDrvSST25VF080BObj[object].status);
}

// *****************************************************************************
/* Function:
    DRV_HANDLE DRV_SST25VF080B_Open
    (
        const SYS_MODULE_INDEX drvIndex,
...
{
    DRV_SST25VF080B_CLIENT_OBJ *clientObj;
    DRV_SST25VF080B_OBJ *dObj;
    ...
    if (drvIndex >= DRV_SST25VF080B_INSTANCES_NUMBER)
    {
        ...
        SYS_DEBUG(0, "DRV_SST25VF080B: Invalid Driver Instance \n");
    ...
    {
        /* The SST25VF080B module should be ready */

        SYS_DEBUG(0, "DRV_SST25VF080B: Was the driver initialized? \n");
        return DRV_HANDLE_INVALID;
    }
    ...
    if(dObj->isExclusive)
    {
        ...
        SYS_DEBUG(0, "DRV_SST25VF080B: Driver already opened exclusively \n"); 
        return ( DRV_HANDLE_INVALID ) ;
    }
    ...
    if((dObj->nClients > 0) && (ioIntent & DRV_IO_INTENT_EXCLUSIVE))
    {
        ...
        SYS_DEBUG(0, "DRV_SST25VF080B: Driver already opened. Cannot be opened exclusively \n");
        return(DRV_HANDLE_INVALID);
    }

    /* Grab client object mutex here */

//    if(OSAL_MUTEX_Lock(&(gDrvSST25VF080BCommonDataObj.mutexClientObjects), OSAL_WAIT_FOREVER) == OSAL_RESULT_TRUE)
    {
        ...
        for(iClient = 0; iClient != DRV_SST25VF080B_CLIENTS_NUMBER; iClient ++)
        {
            if(!gDrvSST25VF080BClientObj[iClient].inUse)
            {
                /* This means we have a free client object to use */
                clientObj = &gDrvSST25VF080BClientObj[iClient];
                clientObj->inUse        = true;
                
//                /* We have found a client object. Release the mutex */
//
//                OSAL_ASSERT(OSAL_MUTEX_Unlock(&(gDrvSST25VF080BCommonDataObj.mutexClientObjects)),
...
//                         "Unable to create client write done semaphore");
                ...
                clientObj->clientStatus = DRV_SST25VF080B_CLIENT_STATUS_READY;
...
// *****************************************************************************
/* Function:
    void DRV_SST25VF080B_Close( DRV_Handle handle );
...
void DRV_SST25VF080B_Close( const DRV_HANDLE handle)
{
    ...
    DRV_SST25VF080B_CLIENT_OBJ  * clientObj;
    DRV_SST25VF080B_OBJ * dObj;
    DRV_SST25VF080B_BUFFER_OBJ * iterator = NULL;

    /* Validate the handle */
    clientObj = _DRV_SST25VF080B_DriverHandleValidate(handle);

    if(clientObj == NULL)
    {
        /* Driver handle is not valid */
        SYS_DEBUG(0, "DRV_SST25VF080B: Invalid Driver Handle \n");
        return;
    }

    dObj = (DRV_SST25VF080B_OBJ *)clientObj->hDriver;
...
//    {
        ...
        while(iterator != NULL)
        {
            if(clientObj == (DRV_SST25VF080B_CLIENT_OBJ *)iterator->hClient)
            {
...
//        SYS_DEBUG(0, "DRV_SST25VF080B: Could not remove client buffer objects \n");
//        clientObj->clientStatus = DRV_SST25VF080B_CLIENT_STATUS_ERROR;
//        return;
//    }
//    
...
//             "Unable to delete client read done semaphore");
    ...
    clientObj->clientStatus = DRV_SST25VF080B_CLIENT_STATUS_CLOSED;
...
// *****************************************************************************
/* Function:
    DRV_SST25VF080B_CLIENT_STATUS DRV_SST25VF080B_ClientStatus (DRV_HANDLE handle )

  Summary:
    Dynamic impementation of DRV_SST25VF080B_ClientStatus client interface function.

  Description:
    This is the dynamic impementation of DRV_SST25VF080B_ClientStatus client interface 
    function.
...
DRV_SST25VF080B_CLIENT_STATUS DRV_SST25VF080B_ClientStatus(DRV_HANDLE handle)
{
    ...
    client = _DRV_SST25VF080B_DriverHandleValidate(handle);
    ...
    if(client == NULL)
    {
        ...
        SYS_DEBUG(0, "DRV_SST25VF080B: Invalid driver handle \n");
        return DRV_SST25VF080B_CLIENT_STATUS_CLOSED;
    }
...
// *****************************************************************************
/* Function:
    void DRV_SST25VF080B_BlockEventHandlerSet
    (
        const DRV_HANDLE handle,
        const DRV_SST25VF080B_EVENT_HANDLER eventHandler,
...
// *****************************************************************************
/* Function:
    SYS_FS_MEDIA_GEOMETRY DRV_SST25VF080B_GeometryGet( DRV_HANDLE handle );

  Summary:
    Returns the geometry of the device.

  Description:
    This API gives the following geometrical details of the SST25VF080B Flash:
    - Media Property
...
SYS_FS_MEDIA_GEOMETRY * DRV_SST25VF080B_GeometryGet( DRV_HANDLE handle )
{
    ...
    DRV_SST25VF080B_CLIENT_OBJ * hClient;
    DRV_SST25VF080B_OBJ *hDriver;

    /* Validate the driver handle */
    hClient = _DRV_SST25VF080B_DriverHandleValidate(handle);
    ...
    hDriver->memoryRegions[2].blockSize = _DRV_SST25VF080B_ERASE_BLOCK_SIZE;	//  4 KByte
...
// *****************************************************************************
/* Function:
    bool DRV_SST25VF080B_MediaIsAttached(DRV_HANDLE handle);
...
// *****************************************************************************
/* Function:
    void _DRV_SST25VF080B_ProcessBufferTask
    (
        DRV_SST25VF080B_OBJ * hDriver,
        DRV_SST25VF080B_BUFFER_OBJ * bufferObj
    )
  ...
  Description:
    Dynamic implementation of the _DRV_SST25VF080B_ProcessBufferTask() function.
...
)
{
    ...
    {
        case DRV_SST25VF080B_BUFFER_PROCESS_INIT:
            {
                /* Set the Buffer status as processing */
                hDriver->queueTail->commandStatus = DRV_SST25VF080B_COMMAND_IN_PROGRESS;

                if (bufferObj->operation == DRV_SST25VF080B_BLOCK_ERASE_WRITE)
                {
                    hDriver->bufferProcessState = DRV_SST25VF080B_ERASE_WRITE;
                    break;
                }
                ...
                if (bufferObj->operation == DRV_SST25VF080B_BLOCK_READ)
                {
                    /* If request is for read, then load the read command and address */
                    /* Add READ op code */
                    hDriver->commandAddressData[0] = SST25VF080B_NORMAL_READ_OP_CODE;
                    /* Add READ address */
                    ...
                    hDriver->bufferProcessState = DRV_SST25VF080B_SEND_READ_CMD_AND_ADDRESS;
                    /* Intentional Fallthrough to the next case */
                }
                else
                {
                    /* If request is for Erase/Write, then load Write Enable command first */
                    hDriver->commandAddressData[0] = SST25VF080B_WREN_OP_CODE;
                    hDriver->bufferProcessState = DRV_SST25VF080B_SEND_WREN_CMD;
                    break;
                }       
            }
        case DRV_SST25VF080B_SEND_READ_CMD_AND_ADDRESS:
            {
                /* Add the buffer in SPI Queue */
                if ((DRV_SPI_BufferAddWrite(hDriver->spiDriverOpenHandle,
                                (uint8_t *)&hDriver->commandAddressData[0], 4, NULL, NULL)) != NULL)
                {
                    /* This means command has been allocated in the SPI queue,
                     * now go to the nexr state */
                    hDriver->bufferProcessState = DRV_SST25VF080B_START_READING;
                }
                else
                {
        ...
        case DRV_SST25VF080B_START_READING:
            {
                if (hDriver->isEraseWrite)
                {
                    hDriver->spiBufferHandle = DRV_SPI_BufferAddRead(hDriver->spiDriverOpenHandle,
                            (uint8_t *)gDrvSST25VF080BBackupBuffer, _DRV_SST25VF080B_ERASE_BLOCK_SIZE, NULL, NULL );
                }
                else
                {
                    hDriver->spiBufferHandle = DRV_SPI_BufferAddRead(hDriver->spiDriverOpenHandle,
                ...
                if(hDriver->spiBufferHandle != NULL)
                {
                    hDriver->bufferProcessState = DRV_SST25VF080B_READ_COMPLETED;
                }
                else
                {
                    break;
        ...
        case DRV_SST25VF080B_READ_COMPLETED:
            {
                ...
                if(DRV_SPI_BUFFER_EVENT_COMPLETE & DRV_SPI_BufferStatus(hDriver->spiBufferHandle))
                {
                    ...
                    if (hDriver->isEraseWrite)
                    {
                        ...
                        hDriver->bufferProcessState = DRV_SST25VF080B_ERASE_WRITE;
                        hDriver->eraseWriteState = _DRV_SST25VF080B_ERASE_WRITE_READ_COMPLETE;
                        break;
                    }
                    else
                    {
                    ...
                    hDriver->bufferProcessState = DRV_SST25VF080B_BUFFER_PROCESS_INIT;
                }
                break;
            }

        case DRV_SST25VF080B_SEND_WREN_CMD:
            {
                /* Add the buffer in SPI Queue */
                ...
                if (hDriver->spiBufferHandle != NULL)
                {
                    ...
                    hDriver->bufferProcessState = DRV_SST25VF080B_WREN_EXECUTION_STATUS_CHECK;
                }
                else
                {
                    /* This means command has not been allocated in the SPI queue,
        ...
        case DRV_SST25VF080B_WREN_EXECUTION_STATUS_CHECK:
            {
                /* Check if the transfer status is success or not */
                if(DRV_SPI_BUFFER_EVENT_COMPLETE & DRV_SPI_BufferStatus(hDriver->spiBufferHandle))
                {
                    ...
                    if (bufferObj->operation == DRV_SST25VF080B_BLOCK_WRITE)
                    {
                        /* It is a WRITE request */
                        hDriver->bufferProcessState = DRV_SST25VF080B_SEND_WRITE_CMD_ADDRESS_AND_DATA;
                    }
                    else if (bufferObj->operation == DRV_SST25VF080B_BLOCK_ERASE) 
                    {
                        /* This means it is an Erase request */
                        hDriver->bufferProcessState = DRV_SST25VF080B_SEND_ERASE_CMD_AND_ADDRESS;
                        break;
                    }
                }
                else
        ...
        case DRV_SST25VF080B_SEND_WRITE_CMD_ADDRESS_AND_DATA:
            {
                /* Enable CS Line */
                SYS_PORTS_PinClear(PORTS_ID_0, hDriver->chipSelectPortChannel, hDriver->chipSelectBitPosition);

                /* Add Write Byte op code */
                hDriver->commandAddressData[0] = SST25VF080B_WRITE_BYTE_OP_CODE;
                /* Add Write address and data pointer */
                hDriver->commandAddressData[1] = (uint8_t)(bufferObj->address >> 16);
                hDriver->commandAddressData[2] = (uint8_t)(bufferObj->address >> 8);
                hDriver->commandAddressData[3] = (uint8_t)(bufferObj->address);
                if (hDriver->isEraseWrite)
                    hDriver->commandAddressData[4] = (uint8_t)(gDrvSST25VF080BBackupBuffer[bufferObj->nCurrentBlocks++]);
                else
                ...
                if (hDriver->spiBufferHandle != NULL)
                {
                    ...
                    hDriver->bufferProcessState = DRV_SST25VF080B_WAIT_FOR_WRITE_OR_ERASE_BUFFER_COMPLETE;
                }
                break;
            }
        case DRV_SST25VF080B_SEND_ERASE_CMD_AND_ADDRESS:
            {
                /* Enable CS Line */
                SYS_PORTS_PinClear(PORTS_ID_0, hDriver->chipSelectPortChannel, hDriver->chipSelectBitPosition);

                /* Add Erase op code */
                hDriver->commandAddressData[0] = SST25VF080B_ERASE_OP_CODE;
                /* Add erase address */
                hDriver->commandAddressData[1] = (uint8_t)(bufferObj->address >> 16);
                hDriver->commandAddressData[2] = (uint8_t)(bufferObj->address >> 8);
                hDriver->commandAddressData[3] = (uint8_t)(bufferObj->address);
        ...
        case DRV_SST25VF080B_WAIT_FOR_WRITE_OR_ERASE_BUFFER_COMPLETE:
            {
                /* Check if the transfer status is success or not */
                if(DRV_SPI_BUFFER_EVENT_COMPLETE & DRV_SPI_BufferStatus(hDriver->spiBufferHandle))
                {
                    ...
                    hDriver->bufferProcessState = DRV_SST25VF080B_SEND_COMMAND_FOR_BUSY_STATUS;
                }
                else
                {
                    break;
        ...
        case DRV_SST25VF080B_SEND_COMMAND_FOR_BUSY_STATUS:
            {
                /* Enable CS Line */
                SYS_PORTS_PinClear(PORTS_ID_0, hDriver->chipSelectPortChannel, hDriver->chipSelectBitPosition);

                /* Add RDSR op code to read the status register */
                hDriver->commandAddressData[0] = SST25VF080B_RDSR_OP_CODE;
                ...
                if (hDriver->spiBufferHandle != NULL)
                {
                    /* This means command has been allocated in the SPI queue,
                     *  now wait for it to be executed in the next state */
                    hDriver->bufferProcessState = DRV_SST25VF080B_READ_BUSY_STATUS;
                }
                else
                {
                    /* This means command has not been allocated in the SPI queue,
        ...
        case DRV_SST25VF080B_READ_BUSY_STATUS:
            {
                ...
                if (hDriver->spiBufferHandle != NULL)
                {
                    /* This means command has been allocated in the SPI queue,
                     *  now wait for it to be executed in the next state */
                    hDriver->bufferProcessState = DRV_SST25VF080B_WAIT_FOR_BUSY_CLEAR;
                }
                else
                {
                    /* This means command has not been allocated in the SPI queue,
        ...
        case DRV_SST25VF080B_WAIT_FOR_BUSY_CLEAR:
            {
                if(DRV_SPI_BUFFER_EVENT_COMPLETE & DRV_SPI_BufferStatus(hDriver->spiBufferHandle))
                {
                    if (!(hDriver->commandAddressData[3] & 0x01))
                    {
                        ...
                        if (hDriver->isEraseWrite)
                        {
                            if (bufferObj->operation == DRV_SST25VF080B_BLOCK_ERASE)
                            {
                                /* This was erase request of the erase write operation. */
                                hDriver->eraseWriteState = _DRV_SST25VF080B_ERASE_WRITE_ERASE_COMPLETE;
                            }
                            else
                            {
                                ...
                                bufferObj->address++;
                                hDriver->eraseWriteState = _DRV_SST25VF080B_ERASE_WRITE_CONT_WRITE;
                            }
        ...
        case DRV_SST25VF080B_ERASE_WRITE:
            {
                switch (hDriver->eraseWriteState)
                {
                    case _DRV_SST25VF080B_ERASE_WRITE_INIT:
                        {
                            ...
                            hDriver->eraseBlockAddress = bufferObj->address / _DRV_SST25VF080B_ERASE_BLOCK_SIZE;

                            /* Compute the start address for the read operation. */
                            bufferObj->address = hDriver->eraseBlockAddress << 12;
                            bufferObj->size    = _DRV_SST25VF080B_ERASE_BLOCK_SIZE;

                            bufferObj->operation = DRV_SST25VF080B_BLOCK_READ;
                            hDriver->bufferProcessState = DRV_SST25VF080B_BUFFER_PROCESS_INIT;
                            break;
                        }

                    case _DRV_SST25VF080B_ERASE_WRITE_READ_COMPLETE:
                        {
                            /* Find the offset in the erase block */
                            uint16_t offset = hDriver->writeAddress % _DRV_SST25VF080B_ERASE_BLOCK_SIZE;
                            uint16_t numBytes = 0;

                            if ((offset + hDriver->numPendingWriteBlocks) > _DRV_SST25VF080B_ERASE_BLOCK_SIZE)
                                numBytes = _DRV_SST25VF080B_ERASE_BLOCK_SIZE - offset;
                            else
                            ...
                            while (numBytes)
                            {
                                gDrvSST25VF080BBackupBuffer[offset++] = *bufferObj->buffer;
                                bufferObj->buffer++;
                                numBytes--;
                            }
                            ...
                            bufferObj->operation = DRV_SST25VF080B_BLOCK_ERASE;
                            hDriver->bufferProcessState = DRV_SST25VF080B_BUFFER_PROCESS_INIT;
                            break;
                        }

                    case _DRV_SST25VF080B_ERASE_WRITE_ERASE_COMPLETE:
                        {
                            ...
                            bufferObj->operation = DRV_SST25VF080B_BLOCK_WRITE;
                            hDriver->bufferProcessState = DRV_SST25VF080B_BUFFER_PROCESS_INIT;
                            break;
                        }

                    case _DRV_SST25VF080B_ERASE_WRITE_CONT_WRITE:
                        {
                            /* transition to either start aai or resume aai states */
                            if (bufferObj->nCurrentBlocks == _DRV_SST25VF080B_ERASE_BLOCK_SIZE)
                            {
                                hDriver->eraseWriteState = _DRV_SST25VF080B_ERASE_WRITE_WRITE_COMPLETE;
                            }
                            else
                            {
                                hDriver->bufferProcessState = DRV_SST25VF080B_BUFFER_PROCESS_INIT;
                    ...
                    case _DRV_SST25VF080B_ERASE_WRITE_WRITE_COMPLETE:
                        {
                            bufferObj->nCurrentBlocks = 0;
                            ...
                            else
                            {
                                ...
                                hDriver->bufferProcessState = DRV_SST25VF080B_BUFFER_PROCESS_INIT;
                                hDriver->eraseWriteState = _DRV_SST25VF080B_ERASE_WRITE_INIT;
...
// ***************************************************************************
/* Function:
    void DRV_SST25VF080B_Tasks ( SYS_MODULE_OBJ object );
...
void DRV_SST25VF080B_Tasks ( SYS_MODULE_OBJ object )
{
    ...
    if(object >= DRV_SST25VF080B_INSTANCES_NUMBER)
    {
        SYS_DEBUG(0, "DRV_SST25VF080B: Invalid module object \n");
        return;
    }

    hDriver = (DRV_SST25VF080B_OBJ*) &gDrvSST25VF080BObj[object];
    ...
    {
        case DRV_SST25VF080B_TASK_OPEN_SPI_DRIVER:
        {
            ...
            if(hDriver->spiDriverOpenHandle != DRV_HANDLE_INVALID)
            {
                ...
                * WRITE Protection of SST Flash in the next state */
                hDriver->state = DRV_SST25VF080B_SEND_WREN_CMD_WRSR;
            }
            else
            {
        ...
        case DRV_SST25VF080B_SEND_WREN_CMD_WRSR:
        {
            ...
            if (hDriver->spiBufferHandle != NULL)
            {
                /* If the command has been queued,
                 * then check if it has been processed in the next state */
                hDriver->state = DRV_SST25VF080B_WREN_EXECUTION_STATUS_CHECK_WRSR;
            }
            else
            {
                /* this means command has not been allocated in the SPI queue,
        ...
        case DRV_SST25VF080B_WREN_EXECUTION_STATUS_CHECK_WRSR:
        {
            /* Check if the transfer status is success or not */
            if(DRV_SPI_BUFFER_EVENT_COMPLETE & DRV_SPI_BufferStatus(hDriver->spiBufferHandle))
            {
                ...
                SYS_PORTS_PinSet(PORTS_ID_0, hDriver->chipSelectPortChannel, hDriver->chipSelectBitPosition);
                hDriver->state = DRV_SST25VF080B_SEND_WRSR_CMD_AND_VALUE;
            }
            else
            {
        ...
        case DRV_SST25VF080B_SEND_WRSR_CMD_AND_VALUE:
        {
            ...
            /* Add WRSR op code */
            hDriver->commandAddressData[0] = SST25VF080B_WRSR_OP_CODE;
            ...
            if (hDriver->spiBufferHandle != NULL)
            {
                /* If the command has been queued,
                 * then check if it has been processed in the next state */
                hDriver->state = DRV_SST25VF080B_WRSR_EXECUTION_STATUS_CHECK;
            }
            else
            {
        ...
        case DRV_SST25VF080B_WRSR_EXECUTION_STATUS_CHECK:
        {
            /* Check if the transfer status is success or not */
            if(DRV_SPI_BUFFER_EVENT_COMPLETE & DRV_SPI_BufferStatus(hDriver->spiBufferHandle))
            {
                ...
                hDriver->state = DRV_SST25VF080B_TASK_PROCESS_QUEUE;
            }
            else
            {
        ...
        case DRV_SST25VF080B_TASK_PROCESS_QUEUE:
        {
            ...
            if(bufferObj != NULL)
            {
                ...
                if(bufferObj->nCurrentBlocks >= bufferObj->size)
                {
                    if (!hDriver->isEraseWrite)
                    {
                        ...
                        bufferObj->commandStatus = DRV_SST25VF080B_COMMAND_COMPLETED;

                        /* If there is a callback registered with client, then call it. */
                        client = (DRV_SST25VF080B_CLIENT_OBJ *)bufferObj->hClient;
                        if(client->eventHandler != NULL)
                        {
                            client->eventHandler(DRV_SST25VF080B_EVENT_BLOCK_COMMAND_COMPLETE,
                                    (DRV_SST25VF080B_BLOCK_COMMAND_HANDLE)bufferObj->commandHandle,
                                    client->context);
                        }
            ...
            {
                _DRV_SST25VF080B_ProcessBufferTask(hDriver, hDriver->queueTail);
            }
            break;
        }
...
// **************************************************************************
/* Function:
    void DRV_SST25VF080B_BlockErase
    (
        const DRV_HANDLE hClient,
        DRV_SST25VF080B_BLOCK_COMMAND_HANDLE * commandHandle,
        uint32_t blockStart,
        uint32_t nBlock
    )
  ...
  Description:
    ...
    The function returns DRV_SST25VF080B_EVENT_BLOCK_HANDLE_INVALID in the
    ...
    driver will issue a DRV_SST25VF080B_EVENT_ERASE_COMPLETE event if the
    erase operation was successfull or DRV_SST25VF080B_EVENT_ERASE_ERROR
    event if the erase operation was not successfull.

    The block start address should be aligned on a
    DRV_SST25VF080B_ERASE_BLOCK_SIZE byte boundary.
...
)
{
    DRV_SST25VF080B_CLIENT_OBJ * clientObj;
    DRV_SST25VF080B_OBJ * hDriver;
    DRV_SST25VF080B_BUFFER_OBJ * bufferObj, * iterator;
    unsigned int iEntry;
    ...
    {
        *commandHandle = DRV_SST25VF080B_BLOCK_COMMAND_HANDLE_INVALID;
    }

    /* Validate the driver handle */
    clientObj = _DRV_SST25VF080B_DriverHandleValidate(hClient);
    if(clientObj == NULL)
    {
    ...
    if(!(clientObj->ioIntent & DRV_IO_INTENT_WRITE))
    {
        ...
        SYS_DEBUG(0, "DRV_SST25VF080B: Driver is not open in Write mode \n");
        return;
    }
    ...
    if((nBlock == 0))
    {
        ...
        SYS_DEBUG(0, "DRV_SST25VF080B: Invalid block size parameter \n");
        return;
    }
    ...
    if(hDriver->queueOccupancy >= hDriver->queueSize)
    {
        ...
        SYS_DEBUG(0, "DRV_SST25VF080B: Queue is full for this driver instance \n");
        return;
    }

    /* Search the buffer pool for a free buffer object */
    for(iEntry = 0 ; iEntry < DRV_SST25VF080B_QUEUE_DEPTH_COMBINED; iEntry ++)
    {
        if(!gDrvSST25VF080BBufferObj[iEntry].inUse)
        {
            /* This means this object is free.
             * Configure the object and then
             * break */
            bufferObj = &gDrvSST25VF080BBufferObj[iEntry];
            bufferObj->hClient = clientObj;
            bufferObj->inUse = true;
            bufferObj->nCurrentBlocks = 0;
            bufferObj->size = nBlock;
            bufferObj->commandHandle    = _DRV_SST25VF080B_MAKE_HANDLE(gDrvSST25VF080BBufferToken, iEntry);
            bufferObj->commandStatus = DRV_SST25VF080B_COMMAND_QUEUED;
            ...
            _DRV_SST25VF080B_UPDATE_BUF_TOKEN(gDrvSST25VF080BBufferToken);
        }
    }

    if(iEntry == DRV_SST25VF080B_QUEUE_DEPTH_COMBINED)
    {
        /* This means we could not find a buffer. This
           will happen if the the DRV_SST25VF080B_QUEUE_DEPTH_COMBINED
           parameter is configured to be less */

        SYS_DEBUG(0, "DRV_SST25VF080B: Insufficient Queue Depth");
        return;
    }
...
// *****************************************************************************
/* Function:
    void DRV_SST25VF080B_BlockRead
    (
        const DRV_HANDLE handle,
        DRV_SST25VF080B_BLOCK_COMMAND_HANDLE * commandHandle,
  ...
  Description:
    ...
    returns DRV_SST25VF080B_EVENT_BLOCK_HANDLE_INVALID in the commandHandle
    ...
    - if the driver handle is invalid
    If the requesting client registered an event callback with the driver, the
    driver will issue a DRV_SST25VF080B_EVENT_BLOCK_COMPLETE event if the
    buffer was processed successfully of DRV_SST25VF080B_EVENT_BLOCK_ERROR
    event if the buffer was not processed successfully.

    The block start address should be aligned on a
    DRV_SST25VF080B_READ_BLOCK_SIZE byte boundary.
...
)
{
    ...
    if(!(clientObj->ioIntent & DRV_IO_INTENT_READ))
    {
        ...
        SYS_DEBUG(0, "DRV_SST25VF080B: Driver is not open in Read mode \n");
        return;
    }
    ...
    if((nBlock == 0) || (targetBuffer == NULL ))
    {
        ...
        SYS_DEBUG(0, "DRV_SST25VF080B: Invalid parameters \n");
        return;
    }
...
// *****************************************************************************
/* Function:
    void DRV_SST25VF080B_BlockWrite
    (
        DRV_HANDLE handle,
        DRV_SST25VF080B_BLOCK_COMMAND_HANDLE * commandHandle,
  ...
  Description:
    ...
    If the requesting client registered an event callback with the driver, the
    driver will issue a DRV_SST25VF080B_EVENT_BLOCK_COMPLETE event if the
    buffer was processed successfully or DRV_SST25VF080B_EVENT_BLOCK_ERROR
    event if the buffer was not processed successfully.

    The block start address should be aligned on a
    DRV_SST25VF080B_WRITE_BLOCK_SIZE byte boundary.
...
)
{
    ...
    if(hDriver->queueOccupancy >= hDriver->queueSize)
    {
        ...
        SYS_DEBUG(0, "DRV_SST25VF080B: Queue is full \n");
        return;
    }
...
// *****************************************************************************
/* Function:
    DRV_SST25VF080B_COMMAND_STATUS DRV_SST25VF080B_CommandStatus
    (
        const DRV_HANDLE handle,
        const DRV_SST25VF080B_BLOCK_COMMAND_HANDLE commandHandle
    );
  ...
  Description:
    ...
    function may return DRV_SST25VF080B_BLOCK_COMMAND_HANDLE_INVALID in a case
  ...
  Remarks:
    Refer to drv_sst25vf080b.h for usage information.
*/
...
)
{
    ...
    if(NULL == _DRV_SST25VF080B_DriverHandleValidate(hClient))
    {
        return DRV_SST25VF080B_COMMAND_ERROR_UNKNOWN;
    }

    /* The upper 16 bits of the buffer handle
     * are the token and the lower 16 bits of the
     * are buffer index into the gDrvSST25VF080BBufferObject
     * array */
    iEntry = commandHandle & 0xFFFF;

    /* Compare the buffer handle with buffer handle
     * in the object */
    if(gDrvSST25VF080BBufferObj[iEntry].commandHandle != commandHandle)
    {
        /* This means that object has been re-used by another
         * request. Indicate that the operation is completed.
         * */
        return (DRV_SST25VF080B_COMMAND_COMPLETED);
    }

    /* Return the last known buffer object status */
    return (gDrvSST25VF080BBufferObj[iEntry].commandStatus);
}
/*******************************************************************************
 End of File
*/
