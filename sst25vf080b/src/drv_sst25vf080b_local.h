/*******************************************************************************
  SST25VF080B SPI Flash Driver Local Data Structures

  Company:
    Microchip Technology Inc.

  File Name:
    drv_sst25vf080b_local.h
...
#ifndef _DRV_SST25VF080B_LOCAL_H
#define _DRV_SST25VF080B_LOCAL_H
...
#include <stdbool.h>
#include <stddef.h>
#include "driver/spi_flash/sst25vf080b/drv_sst25vf080b.h"
#include "driver/spi_flash/sst25vf080b/src/drv_sst25vf080b_variant_mapping.h"
#include "osal/osal.h"
...
/* Write Disable */
#define SST25VF080B_WRDI_OP_CODE            0x04
/* Write Enable */
#define SST25VF080B_WREN_OP_CODE            0x06
/* Write Status Register */
#define SST25VF080B_WRSR_OP_CODE            0x01
/* Erase 4 KByte of Memory Area */
#define SST25VF080B_ERASE_OP_CODE           0x20
/* Byte program */
#define SST25VF080B_WRITE_BYTE_OP_CODE      0x02
/* Read Memory at Normal Speed */
#define SST25VF080B_NORMAL_READ_OP_CODE     0x03
/* Read Memory at High Speed */
#define SST25VF080B_HIGH_SPEED_READ_OP_CODE 0x0B
/* Read Status Register */
#define SST25VF080B_RDSR_OP_CODE            0x05
/* Auto Address Increment Programming */
#define SST25VF080B_AAI_OP_CODE             0xAD
...
#define _DRV_SST25VF080B_BUF_TOKEN_MAX         (0xFFFF)
#define _DRV_SST25VF080B_MAKE_HANDLE(token, index) ((token) << 16 | (index))
#define _DRV_SST25VF080B_UPDATE_BUF_TOKEN(token) \
{ \
    (token)++; \
    (token) = ((token) == _DRV_SST25VF080B_BUF_TOKEN_MAX) ? 0: (token); \
}
...
// *****************************************************************************
/* _DRV_SST25VF080B_ERASE_WRITE_STATES
  ...
  Description
    ...
    task routine can have while in the DRV_SST25VF080B_ERASE_WRITE state.
...
typedef enum {
    ...
    _DRV_SST25VF080B_ERASE_WRITE_INIT,
    
    /* Read substate of the read-modify-write cycle */
    _DRV_SST25VF080B_ERASE_WRITE_READ_COMPLETE,
    
    /* Modify/Erase substate of the read-modify-write cycle */
    _DRV_SST25VF080B_ERASE_WRITE_ERASE_COMPLETE,
    
    /* Write substate of the read-modify-write cycle */
    _DRV_SST25VF080B_ERASE_WRITE_CONT_WRITE,
    
    /* Write Completed substate of the read-modify-write cycle */
    _DRV_SST25VF080B_ERASE_WRITE_WRITE_COMPLETE

} _DRV_SST25VF080B_ERASE_WRITE_STATES;

// *****************************************************************************
/* DRV_SST25VF080B_BUFFER_PROCESS_STATE
...
typedef enum{
    ...
    DRV_SST25VF080B_BUFFER_PROCESS_INIT,

    /* Send write enable command */
    DRV_SST25VF080B_SEND_WREN_CMD,

    /* Check if write has been enabled */
    DRV_SST25VF080B_WREN_EXECUTION_STATUS_CHECK,

    /* Send Write command along with write address and data pointer */
    DRV_SST25VF080B_SEND_WRITE_CMD_ADDRESS_AND_DATA,

    /* Send erase command and address */
    DRV_SST25VF080B_SEND_ERASE_CMD_AND_ADDRESS,

    /* Send read command and address */
    DRV_SST25VF080B_SEND_READ_CMD_AND_ADDRESS,

    /* Start the read operation */
    DRV_SST25VF080B_START_READING,

    /* Wait for Write/Erase buffer completion */
    DRV_SST25VF080B_WAIT_FOR_WRITE_OR_ERASE_BUFFER_COMPLETE,

    /* Send command to read the status register */
    DRV_SST25VF080B_SEND_COMMAND_FOR_BUSY_STATUS,

    /* Read the status of Busy Bit */
    DRV_SST25VF080B_READ_BUSY_STATUS,

    /* Wait until BUSY bit is clear */
    DRV_SST25VF080B_WAIT_FOR_BUSY_CLEAR,

    /* Read operation is completed */
    DRV_SST25VF080B_READ_COMPLETED,

    /* Erase write main state */
    DRV_SST25VF080B_ERASE_WRITE,

}DRV_SST25VF080B_BUFFER_PROCESS_STATE;

// *****************************************************************************
/* SST25VF080B Driver task states

  Summary
    Lists the different states that SST25VF080B task routine can have.

  Description
    This enumeration lists the different states that SST25VF080B task routine
    can have.
...
{
    /* Open SPI driver */
    DRV_SST25VF080B_TASK_OPEN_SPI_DRIVER,

    /* This state is to send the WREN command to be able to modify
       SST Status register */
    DRV_SST25VF080B_SEND_WREN_CMD_WRSR,

    /* Check if write has been enabled */
    DRV_SST25VF080B_WREN_EXECUTION_STATUS_CHECK_WRSR,

    /* Send write status register command and status register value */
    DRV_SST25VF080B_SEND_WRSR_CMD_AND_VALUE,

    /* Check if Status register is written */
    DRV_SST25VF080B_WRSR_EXECUTION_STATUS_CHECK,

    /* Process Queue */
    DRV_SST25VF080B_TASK_PROCESS_QUEUE

} DRV_SST25VF080B_DATA_OBJECT_STATE;

// *****************************************************************************
/* SST25VF080B Driver Operations

  Summary
    Lists the different operations that SST25VF080B driver can do.

  Description
    This enumeration lists the different operations that SST25VF080B driver can
    do.
...
{
    /* Block Read */
    DRV_SST25VF080B_BLOCK_READ,

    /* Block Write */
    DRV_SST25VF080B_BLOCK_WRITE,

    /* Block Erase */
    DRV_SST25VF080B_BLOCK_ERASE,

    /* Erase Write */
    DRV_SST25VF080B_BLOCK_ERASE_WRITE

} DRV_SST25VF080B_BLOCK_OPERATION;

// *****************************************************************************
/* SST25VF080B SPI Flash Driver Buffer Object
...
typedef struct _DRV_SST25VF080B_BUFFER_OBJ
{
    ...
    DRV_SST25VF080B_BLOCK_OPERATION operation;
            
    /* Client that owns this buffer */
    void * hClient;
    
    /* Status of the Buffer */
    DRV_SST25VF080B_COMMAND_STATUS commandStatus;

    /* Current command handle of this buffer object */
    DRV_SST25VF080B_BLOCK_COMMAND_HANDLE commandHandle;

    /* Next buffer pointer */
    struct _DRV_SST25VF080B_BUFFER_OBJ * next;

    /* Previous buffer pointer */
    struct _DRV_SST25VF080B_BUFFER_OBJ * previous;

} DRV_SST25VF080B_BUFFER_OBJ;

// *****************************************************************************
/* SST25VF080B Driver Global Instances Object
  ...
  Description:
    ...
    instances of the SST25VF080B driver.
...
} DRV_SST25VF080B_COMMON_DATA_OBJ;

// *****************************************************************************
/* SST25VF080B SPI Flash Driver Instance Object

  Summary:
    Object used to keep any data required for an instance of the SST25VF080B SPI
    Flash driver.
...
typedef struct
{
     ...
     * queueTail */
    DRV_SST25VF080B_BUFFER_OBJ  *queueTail;
    ...
    /* State of the task */
    DRV_SST25VF080B_DATA_OBJECT_STATE    	state;

    /* State of the buffer object transfer */
    DRV_SST25VF080B_BUFFER_PROCESS_STATE    		bufferProcessState;
    ...
    _DRV_SST25VF080B_ERASE_WRITE_STATES eraseWriteState;
...
// *****************************************************************************
/* SST25VF080B SPI Flash Driver Client Object
...
typedef struct
{
    ...
    DRV_SST25VF080B_EVENT_HANDLER eventHandler;
...
} DRV_SST25VF080B_CLIENT_OBJ;

/* This is the array of SST25VF080B SPI Flash Driver Buffer objects. */
extern DRV_SST25VF080B_BUFFER_OBJ gDrvSST25VF080BBufferObj[DRV_SST25VF080B_QUEUE_DEPTH_COMBINED];

extern uint16_t gDrvSST25VF080BBufferToken;

extern DRV_SST25VF080B_CLIENT_OBJ * _DRV_SST25VF080B_DriverHandleValidate(DRV_HANDLE handle);

#endif //#ifndef _DRV_SST25VF080B_LOCAL_H
